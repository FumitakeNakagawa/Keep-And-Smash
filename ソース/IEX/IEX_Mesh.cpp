...
//------------------------------------------------------
//		レイピック(scale,pos,angle変更可能)
//------------------------------------------------------
int iexMesh::RayPick2(Vector3& out, Vector3& pos, Vector3& vec, float &Dist)
{
	int	ret;

	//	ワールド変換行列取得
	Matrix mat = this->TransMatrix;

	//	逆行列作成
	Matrix inv;
	D3DXMatrixInverse(&inv, NULL, &mat);

	//  ワールド位置をローカル変換する
	Vector3	workP;
	workP.x = pos.x*inv._11 + pos.y*inv._21 + pos.z*inv._31 + inv._41;
	workP.y = pos.x*inv._12 + pos.y*inv._22 + pos.z*inv._32 + inv._42;
	workP.z = pos.x*inv._13 + pos.y*inv._23 + pos.z*inv._33 + inv._43;

	Vector3	workV;
	workV.x = vec.x*inv._11 + vec.y*inv._21 + vec.z*inv._31;
	workV.y = vec.x*inv._12 + vec.y*inv._22 + vec.z*inv._32;
	workV.z = vec.x*inv._13 + vec.y*inv._23 + vec.z*inv._33;
	workV.Normalize();

	//	レイピック
	Vector3	workOut;
	ret = RayPick(&workOut, &workP, &workV, &Dist);

	//	位置・法線補正
	if (ret != -1)
	{
		out.x = workOut.x * mat._11 + workOut.y * mat._21 + workOut.z * mat._31 + mat._41;
		out.y = workOut.x * mat._12 + workOut.y * mat._22 + workOut.z * mat._32 + mat._42;
		out.z = workOut.x * mat._13 + workOut.y * mat._23 + workOut.z * mat._33 + mat._43;
	}
	return ret;
}
